  ...
  methods: {
      ...
      if (event.metaKey) {
        ...
        if (sele.node()) {
          ...
          if (keyName === 'Tab') {
            // 添加子节点
            d3.event.preventDefault()
            this.editNew(this.add(im, { name: '' }), seleDepth + 1, pNode)
            ...
            if (pNode === this.$refs.content) {
              ...
              this.editNew(this.insert(im, { name: '' }, 1), seleDepth, pNode)
            }
    ...
    fObjMousedown(d, i, n) {
      ...
      if (edit && edit !== clickedNode) {
        const f = d3
          .selectAll('foreignObject')
          .filter((d, i, n) => n[i].parentNode === edit)
    ...
    dragended(d, i, n) {
      ...
      if (newParentNode) {
      ...
      } else if (d.depth === 0 || Math.abs(d.px) < root.nodeHeight) {
        // 平移距离不足以调换兄弟节点顺序时复原
        dragback(d, draggedNode)
        fObjectClick(d, i, n)
      } else {
        ...
        const draggedBrotherNodes = draggedParentNode
          .selectAll(`g.depth_${dPdata.depth + 1}`)
          .filter((a, i, n) => draggedNode !== n[i])
        if (!draggedBrotherNodes.nodes()[0]) {
          ...
          draggedBrotherNodes.each((b, i, n) => {
            if (b.x > d.x && b.x > a.x1 && b.x < d.x + d.px) {
              // 新哥哥节点
              a.x1 = b.x
              a.b1 = b.data
              a.n1 = n[i]
            }
            if (b.x < d.x && b.x < a.x0 && b.x > d.x + d.px) {
              // 新弟弟节点
              a.x0 = b.x
              a.b0 = b.data
              a.n0 = n[i]
    ...
    gEllTransform(d) {
      return `translate(${d.data.size[1] + 8 - this.xSpacing},${d.data.size[0] / 2})`
    },
    path(d) {
      return `${this.link({
        source: [
    ...
    updateNode(update) {
      ...
      update.each((d, k, m) => {
        ...
        const foreign = node
          .selectAll('foreignObject')
          .filter((d, i, n) => n[i].parentNode === m[k])
...
button.icon {
  ...
  i {
    ...
    &.gps {
      background-image: url(../../public/icons/24px/crosshairs-gps.png);
    }
    &.fitView {
      background-image: url(../../public/icons/24px/fit-to-page-outline.png);
    }
    &.download {
      background-image: url(../../public/icons/24px/download.png);
    }
    &.undo {
      background-image: url(../../public/icons/24px/undo.png);
    }
    &.redo {
      background-image: url(../../public/icons/24px/redo.png);
    }
    &.close {
      background-image: url(../../public/icons/24px/close.png);
    }
  }
}
...
div#mindmap {
  ...
  svg {
    ...
    .for(@sw) when(@i <= length(@strokeWidths)) {
      path {
        stroke-width: @sw;
      }
    }
  ...
  div.buttonList {
    ...
    i {
      filter: invert(25%) sepia(40%) saturate(5050%) hue-rotate(227deg) brightness(78%)
        contrast(74%);
    }
          ...
          .option {
            ...
            .icon {
              ...
              i {
                ...
                &.code-json {
                  background-image: url(../../public/icons/36px/code-json.png);
                }
                &.image {
                  background-image: url(../../public/icons/36px/image.png);
                }
                &.markdown {
                  background-image: url(../../public/icons/36px/markdown.png);
                }
              }
            }
